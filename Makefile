...
DECB = $(SHELF)/bin/decb
STORY ?= $(HOME)/development/emulator/frotz/ZORK1.DAT
DISK ?= $(shell echo $(STORY) | sed 's/.*\///;s/\.[^.]*$$//').dsk
...
$(DISK): cocozip.bin boot.bin $(STORY)
	...
	dd if=$(STORY) of=$(DISK) bs=256 count=252 seek=36 conv=notrunc
	...
	dd if=$(STORY) of=$(DISK) bs=256 skip=252 seek=324 conv=notrunc

clean:
	rm -f cocozip.bin boot.bin *.dsk *.map *.list
